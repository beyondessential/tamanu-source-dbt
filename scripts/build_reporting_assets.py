...
def main():
    ...
    try:
        ...
        # Build dbt models and generate documentation for each supported language
        cprint("Building dbt models for all supported languages", "info")
        for i, language in enumerate(supported_languages):
            cprint(f"Building models for language: {language}", "info")

            # Build models with language-specific vars
            execute_command(f'dbt run --profiles-dir config --vars "{{language: {language}}}"')

            # Generate documentation only once (for the first language)
            if i == 0:
                cprint("Generating documentation", "info")
            ...
            cprint(f"Completed build for language: {language}", "success")
