...
def main():
    ...
    execute_command(f"dbt run --target {target} --profiles-dir config")
    execute_command(f"dbt docs generate --target {target} --profiles-dir config")
    execute_command(f"dbt compile --target {target} --profiles-dir config")
