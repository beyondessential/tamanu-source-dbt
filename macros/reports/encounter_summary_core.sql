...
{#- BL-008: the candidate bounds, compared against the stored character(19) columns.
    ...
    instead of reading the table. They do NOT fix the estimate: the exact predicates are
    still here and still unestimable, so the scope CTE is costed at a fraction of its true
    size -- measured on a populated fixture, 3 rows against 183, and 6 against 783.
    Everything downstream still plans against that wrong number, which is why BL-009 makes
    the notes join index-reachable outright rather than wait for a better estimate.
