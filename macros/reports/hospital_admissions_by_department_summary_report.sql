...
select
    ...
    -- so each event count filters to the month the event itself falls in. discharge,
    -- death and transfer_out are events at the END of an episode, so they belong to the
    -- month the episode ended -- matching the definitions shipped with these reports.
    -- admission and transfer_in happen at the start and are counted there. An episode's
    -- end is the next episode's start, so a move now lands in one month as both a
    -- transfer out and a transfer in.
...
-- No `rm.month <= current_date` guard here, unlike -by-area and -by-location: those
-- report a month an episode merely spans, so an open episode would run their spine to
-- its end. The `having` below already restricts this report to months an episode started
-- or ended in, and dropping future months would lose a future-dated admission entirely.
where
    case
