select
    ...
    to_char(start_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('encounterStartDateTime', 'Encounter start date and time') }}",
    to_char(end_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('encounterEndDateTime', 'Encounter end date and time') }}",
