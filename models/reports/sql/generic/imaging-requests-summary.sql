...
select
    to_char(rd.date, '{{ var("date_format") }}') as "{{ translate_string('', 'Date') }}",
    ir.facility as "{{ translate_string('general.localisedField.facility.label', 'Facility') }}",
    ir.department as "{{ translate_string('general.localisedField.departmentId.label', 'Department') }}",
    ir.imaging_type as "{{ translate_string('imaging.imagingType.label', 'Imaging type') }}",
    count(distinct ir.request_id) filter (where ir.requested_datetime::date = rd.date) as "{{ translate_string('', 'Total new requests') }}",
    ...
    ) as "{{ translate_string('', 'Total requests with a status of pending') }}",
    count(distinct ir.request_id) filter (
        where ir.completed_datetime::date = rd.date
    ) as "{{ translate_string('', 'Total requests completed') }}"
