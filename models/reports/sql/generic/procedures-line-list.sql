select
    ...
    to_char(procdure_start_time, '{{ var("time_format") }}') as "{{ translate_string('', 'Procedure start (date and time)') }}",
    to_char(procedure_end_time, '{{ var("time_format") }}') as "{{ translate_string('', 'Procedure end (date and time)') }}",
