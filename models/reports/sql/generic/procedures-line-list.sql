select
    ...
    encounter_facility as "{{ translate_string('facilityName', 'Encounter facility') }}",
    encounter_department as "{{ translate_string('departmentName', 'Encounter department') }}",
    encounter_type as "{{ translate_string('encounterType', 'Encounter type') }}",
    to_char(encounter_start_datetime, '{{ var("datetime_format") }}') as "{{ translate_string('encounterStartDate', 'Encounter start date') }}",
    to_char(encounter_end_datetime, '{{ var("datetime_format") }}') as "{{ translate_string('encounterEndDate', 'Encounter end date') }}",
    ...
    to_char(procedure_start_time, '{{ var("time_format") }}') as "{{ translate_string('procedureStartDatetime', 'Procedure start (date and time)') }}",
    to_char(procedure_end_time, '{{ var("time_format") }}') as "{{ translate_string('procedureEndDatetime', 'Procedure end (date and time)') }}",
