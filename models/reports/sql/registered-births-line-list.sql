select
    to_char(registration_date, '{{ var("date_format") }}') as "{{ translate_label('birthRegisteredDateTime', 'Registration date') }}",
    ...
    to_char(birth_time, '{{ var("time_format") }}') as "{{ translate_label('birthTimeOfBirth', 'Time of birth') }}",
