select
    ...
    to_char(admission_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('admissionDateTime', 'Admission date and time') }}",
    admission_status as "{{ translate_label('admissionStatus', 'Admission status') }}",
    to_char(discharge_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('dischargeDateTime', 'Discharge date and time') }}",
