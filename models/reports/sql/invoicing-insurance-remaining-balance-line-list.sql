select
    ...
    to_char(i.admission_datetime::date, '{{ var("date_format") }}') as "{{ translate_label('admissionDate','Admission date') }}",
    to_char(i.discharge_datetime::date, '{{ var("date_format") }}') as "{{ translate_label('dischargeDate','Discharged date') }}",
