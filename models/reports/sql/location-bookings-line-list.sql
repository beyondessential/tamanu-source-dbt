select
    ...
    to_char(booking_start_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('bookingStartDateTime', 'Booking start date and time') }}",
    to_char(booking_end_datetime, '{{ var("datetime_format") }}') as "{{ translate_label('bookingEndDateTime', 'Booking end date and time') }}",
    case
