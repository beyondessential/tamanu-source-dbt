with encounters_in_scope as (
    ...
    where
        ...
        and case
            ...
            else e.start_datetime >= {{ parameter('fromDate', default_value='2024-01-01', data_type='date') }}
        end
        and case
            when {{ parameter('toDate', default_value='2024-01-31', data_type='date') }} is null then true
            else e.start_datetime <= {{ parameter('toDate', default_value='2024-01-31', data_type='date') }}
        end
        and case
...
encounter_history_consolidated as (
    ...
    join encounters_in_scope eis on eis.encounter_id = eh.encounter_id
    join {{ ref('users') }} actor on actor.id = eh.updated_by_id
    join {{ ref('users') }} clinician on clinician.id = eh.clinician_id
...
encounter_diagnoses as (
    ...
    join encounters_in_scope eis on eis.encounter_id = ed.encounter_id
    join {{ ref('reference_data') }} d on d.id = ed.diagnosis_id
...
encounter_prescriptions as (
    ...
    join encounters_in_scope eis on eis.encounter_id = ep.encounter_id
...
encounter_vaccinations as (
    ...
    join encounters_in_scope eis on eis.encounter_id = av.encounter_id
    join {{ ref('vaccine_schedules') }} sv on sv.id = av.scheduled_vaccine_id
    join {{ ref('reference_data') }} v on v.id = sv.vaccine_id
...
encounter_procedures as (
    ...
    join encounters_in_scope eis on eis.encounter_id = p.encounter_id
    left join {{ ref('reference_data') }} proc on proc.id = p.procedure_type_id
    left join {{ ref('locations') }} loc on loc.id = p.location_id
...
encounter_lab_requests as (
    ...
    join encounters_in_scope eis on eis.encounter_id = lr.encounter_id
...
imaging_request_areas as (
    ...
    join encounters_in_scope eis on eis.encounter_id = ir.encounter_id
    left join {{ ref('imaging_request_areas') }} ira on ira.imaging_request_id = ir.id
    left join {{ ref('reference_data') }} area on area.id = ira.area_id
...
invoice_data as (
    ...
    from {{ ref('invoices') }} i
    join encounters_in_scope eis on eis.encounter_id = i.encounter_id
    left join {{ ref('invoice_items') }} ii on ii.invoice_id = i.id
    left join {{ ref('invoice_products') }} ip on ip.id = ii.product_id
...
join {{ ref('patients') }} p on p.id = eis.patient_id
join {{ ref('locations') }} l on l.id = eis.location_id
join {{ ref('departments') }} dp on dp.id = eis.department_id
left join {{ ref('location_groups') }} lg on lg.id = l.location_group_id
left join {{ ref('users') }} c on c.id = eis.clinician_id
join encounter_changes ec on ec.encounter_id = eis.encounter_id
left join {{ ref('triages') }} t on t.encounter_id = eis.encounter_id
left join {{ ref('discharges') }} d on d.encounter_id = eis.encounter_id
left join {{ ref('patient_additional_data') }} pad on pad.patient_id = eis.patient_id
left join {{ ref('reference_data') }} eth on eth.id = pad.ethnicity_id
left join {{ ref('reference_data') }} bt on bt.id = eis.patient_billing_type_id
left join {{ ref('reference_data') }} am on am.id = t.arrival_mode_id
left join {{ ref('reference_data') }} dd on dd.id = d.disposition_id
left join encounter_diagnoses ed on ed.encounter_id = eis.encounter_id
left join encounter_prescriptions ep on ep.encounter_id = eis.encounter_id
left join encounter_vaccinations ev on ev.encounter_id = eis.encounter_id
left join encounter_procedures epr on epr.encounter_id = eis.encounter_id
left join encounter_lab_requests elr on elr.encounter_id = eis.encounter_id
left join encounter_imaging_requests eir on eir.encounter_id = eis.encounter_id
left join encounter_notes en on en.encounter_id = eis.encounter_id
left join invoice_data invd on invd.encounter_id = eis.encounter_id
where
    case
