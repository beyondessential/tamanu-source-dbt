...
-- procedure_type_source_value is the branch discriminator, so imaging and procedures stay
-- distinguishable in one shared table.
--
-- The imaging branch does not join imaging_results -- procedure_datetime is the request's
-- own timestamp, not a completion event, and is_completed is read straight off
...
procedure_branch as (
    select
        ...
        -- the procedure's own location, not the encounter's care_site_id -- a procedure can
        -- be performed somewhere other than the ward the patient is admitted to (e.g. a
        -- theatre)
        p.location_id,
...
-- comment above; the CDM has no dedicated imaging domain). No join to imaging_results:
-- procedure_datetime is when the study was requested, not completed, and is_completed is
-- the request's own recorded status, not a fact derived from a result.
imaging_branch as (
    select
        ...
        -- the requesting clinician, not who (if anyone) completed it -- this branch carries
        -- no completion-side fact
        ir.requested_by_id as provider_id,
        ir.encounter_id as visit_occurrence_id,

        -- imaging_requests' own location -- carried raw like the procedure branch's, though
        -- it is not reliably populated in practice; resolving facility is a consumer-layer
        -- concern either way
        ir.location_id,
        ...
        -- the request's own status, never NULL -- imaging_requests.status is a mandatory
        -- lifecycle field, but this stays defensive on the same basis as the procedure
        -- branch's coalesce above
...
select * from procedure_branch
union all
select * from imaging_branch
