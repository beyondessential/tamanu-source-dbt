...
select
    ...
    p.date_of_death,
    user.display_name as attending_clinician,
    primary_condition.name as primary_cause_condition,
    case
        when pdd.primary_cause_time_after_onset is null or pdd.primary_cause_time_after_onset = 0 then '0 minutes'
        when mod(pdd.primary_cause_time_after_onset, (60 * 24 * 365)) = 0 then concat(pdd.primary_cause_time_after_onset / (60 * 24 * 365), ' years')
        when mod(pdd.primary_cause_time_after_onset, (60 * 24 * 30)) = 0 then concat(pdd.primary_cause_time_after_onset / (60 * 24 * 30), ' months')
    ...
    initcap(pdd.recent_surgery) as recent_surgery,
...
join {{ ref("patients") }} p on p.id = pdd.patient_id
...
left join {{ ref("reference_data") }} other_condition_4 on other_condition_4.id = cdc.other_conditions[4]
