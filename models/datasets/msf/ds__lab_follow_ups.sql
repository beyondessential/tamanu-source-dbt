...
with patient_with_ncd_conditions as (
	...
	where pc.is_resolved = false
		and pc.condition_id in (
			'Diagnosis-Hypothyroidism', 'Diagnosis-Diabetestype1', 'Diagnosis-Diabetestype2', 
			'Diagnosis-Hypertension', 'Diagnosis-Chronickidneydisease',
			'Diagnosis-Cardiovasculardisease', 'Diagnosis-Heartfailure'
		)
	group by patient_id
),
lab_test_results as (
	select 
		...
		lt.result::numeric as test_result,
		case 
	...
	where lt.completed_datetime is not null
		...
		and (lr.lab_test_panel_request_id::text in ('labTestPanel-Urinealysis', 'labTestPanel-CBC')
			or lt.lab_test_type_id in (
				'labTestType-HbA1c', 'labTestType-Creatinine', 'labTestType-Sodium', 'labTestType-Potassium',
				'labTestType-Triglycerides', 'labTestType-TotalCholesterol', 'labTestType-ALT', 'labTestType-TSH',
		...
		*,
		case test_id
			when 'labTestType-HbA1c' then
				case when (age >= 65 and test_result < 8) or (age < 65 and test_result < 7) then 12 else 3 end
		...
		case test_id
			when 'labTestType-HbA1c' then
				case 
					when age >= 65 and test_result < 8 then 'HbA1c < 8 (if 65yo or older) every 6-12 months'
					when age >= 65 and test_result >= 8 then 'HbA1c > 8 (if 65yo or older) every 3 months'
					when age < 65 and test_result < 7 then 'HbA1c <7 if under 65yo every 6-12 months'
					when age < 65 and test_result >= 7 then 'HbA1c >7 (if under 65yo) every 3 months'
					else 'After medication Check every 3 months'
				end
			when 'labTestType-Creatinine' then
				case when is_normal and not has_ckd then 'Normal every 12 months'
					 else 'If outside of normal, or CKD diagnosis, every 3 months' end
			when 'labTestType-Sodium' then
				case when is_normal and not has_ckd then 'Normal every 12 months'
					 else 'If outside of normal, or CKD diagnosis, every 3 months' end
			when 'labTestType-Potassium' then
			...
			when 'labTestType-TSH' then
				case when is_normal then 'Stable (TSH 0.4-4): Yearly'
					 else '2-3 months if instable/after medication change' end
			when 'labTestType-GlucoseFasting' then
				case when not has_diabetes then 'Annually (exclude patients with Diabetes)' else null end
			else 'Annually'
...
select 
	...
	test_date,
	follow_up_frequency,
	test_date + (follow_up_months || ' months')::interval as follow_up_due_date,
	case 
		when follow_up_months is null then 'N/A'
		when current_date > test_date + (follow_up_months || ' months')::interval then 'Over due'
		else 'Upcoming' 
