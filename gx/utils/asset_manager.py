...
def create_data_assets_and_batches(data_source, sql_files):
    ...
    counts = {"added_assets": 0, "skipped_assets": 0, "added_batches": 0, "skipped_batches": 0}
    ...
    return counts["added_assets"], counts["skipped_assets"], counts["added_batches"], counts["skipped_batches"]


def print_summary(added_assets, skipped_assets, added_batches, skipped_batches, 
                 added_suites, skipped_suites, failed_suites, total_files):
    """Print setup summary"""
    print(f"\n{'='*50}")
    print("SUMMARY")
    print(f"{'='*50}")
    for label, added, skipped in [
        ...
        print(f"{label}: ✓ {added} added, ⚪ {skipped} skipped" + (f", ✗ {failed_suites} failed" if label == "Expectation Suites" else ""))
