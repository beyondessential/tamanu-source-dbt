...
|---|---|---|
| `appointmentId` | text | The appointment's id, for cross-referencing against other reports |
...
| `appointmentCreatedDateTime` | text | When the appointment was booked, in the viewer's timezone — distinct from when it is scheduled for |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-17 | Maui team | Added `appointmentId` as the first column and `appointmentCreatedDateTime` as the last (MAUI-6905). `created_datetime` is new in `base__outpatient_appointments` and threaded through the dataset's scope CTE. |
