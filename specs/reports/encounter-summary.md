...
- **BL-008:** The date range is filtered **twice**. The exact predicate compares the
  ...
  **The candidate bounds fix the scan, not the estimate.** This clause originally claimed
  they fixed both. They do not: the exact predicates remain, remain unestimable, and the
  scope CTE is still costed at a fraction of its true size — measured on a populated
  fixture, 3 rows against 183 actual, and 6 against 783. That is why BL-009 could not
  simply wait for the aggregate CTEs to re-plan themselves.
