...
- **BL-012:** One row per change-log entry per registration, ordered by `logged_at` and keyed by
  the log's own `changelog_id`, which the history emits: entries sharing an instant are both kept,
  the log carrying no intra-instant order to collapse them by and a collapse being able to discard
  the entry holding a transition. Entries are scoped to the registrations `clinical__episode`
  models, so a recorded-in-error enrolment (BL-002) contributes no history and every history row
  belongs to an episode (AC-014).
- **BL-013:** `registration_status` and `clinical_status_id` are the values as at that entry, read
  from the logged record snapshot.
- **BL-014:** The current state also appears at the history's latest instant, so a status held now
  is visible without joining back to `clinical__episode`. Where an entry at that instant already
  says what the registration says now, that entry stands for current state and names the user who
  acted; where none does — a log missing an entry — a synthetic row is kept alongside them and
  sorts last, so the divergence shows without discarding a logged change BL-004 may need.
...
- **BL-016:** Sourced only from `bases/` (D10) — the change-log base already excludes the test
  ...
  from the synthetic current-state row, which BL-004 depends on, and a null `changelog_id` marks
  the same row for AC-013.
...
- **BL-022:** Enrolment facts — registration status and datetime, program registry, clinical
  ...
  `clinical_status_id` is consequently the registration's own foreign key rather than the id of
  the joined status row, so a registration pointing at a status since deleted keeps the id and
  loses only the name (AC-028). The registration does hold that status id, and reporting it is
  truer than blanking it; the pre-rebase dataset blanked both because it read the id off the
  join.
...
| AC-013 | `changelog_id` is unique among the history's logged rows, and each registration has at most one synthetic current-state row (a null `changelog_id`) | BL-012, BL-014 | singular test |
| AC-014 | Every history `episode_id` appears in `clinical__episode`, without exception | BL-012 | singular test |
| AC-015 | At each registration's latest history instant, some row carries `clinical__episode`'s current status | BL-014 | singular test |
...
| AC-028 | In `ds__patient_program_registrations`, a row with a `clinical_status_id` but no `clinical_status` is one whose status is absent from `bases/program_registry_clinical_statuses`, and never one the rebase blanked | BL-022 | singular test |
...
| Date | Author | Change |
|---|---|---|
| 2026-08-24 | Maui team | Same-instant change-log entries are no longer collapsed (BL-012). The log's id is a uuid, so there is no last-write-wins to collapse by, and an arbitrary tie-break could discard the entry holding a transition — an exit BL-004 then never sees. The history emits `changelog_id` and AC-013 keys on it instead; BL-014 asks whether *anything* at the last logged instant matches current state rather than picking one row out of it, and AC-015 is asked of the instant for the same reason. BL-022 records that the rebased dataset reports the registration's own `clinical_status_id` rather than the joined status row's, with AC-028 pinning the one case where the two differ. |
