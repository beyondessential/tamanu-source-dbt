...
| **Status** | `review` |
| **Owner** | Maui team |
| **Repo** | `tamanu-source-dbt` (branch line `2.55`) |
...
|---|---|---|---|
| OQ-001 | Whether a "deliveries by maternal age" disaggregation is in scope for a later revision. `patient_birth_data` is keyed on the newborn and has no direct link to the mother's age (only via nullable `patient_additional_data.mother_id`), and `age_group__aihw_maternal_age` — which bands maternal age — does not exist on this branch line (it is on `2.57` and above), so this model could not call it here even if the join were resolved | @maui-team | unscheduled |
...
| `age_group__aihw_maternal_age`, `diagnosis__icd10_obstetric_block` | Maternity macros that exist on `2.57` and above but **not** on this branch line. Neither is called by this model; both remain available to a consumer's data table on the lines that carry them — see OQ-001 |
