...
- **BL-013:** `insurer_payment` is the same net calculation restricted to payments linked to an `invoice_insurer_payments` row (`sum(payments) − sum(refunds)`), with no `status` filter — `invoice_payments.amount` is the amount actually paid and the insurer status is derived from it, so rejected rows contribute 0 and partial rows their real value (mirroring Tamanu's `getSpecificInsurerPaymentRemainingBalance`). Computed here in the shared ephemeral and consumed by `clinical__cost.paid_by_payer`; `ds__encounter_invoices` does not project it.
