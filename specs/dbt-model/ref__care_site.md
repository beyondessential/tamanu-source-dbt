...
both Tamanu **departments** (the organizational care unit) and **locations**
(the room/bed a segment takes place at) as care sites, discriminated by `care_site_type`.
Gives `clinical__` models a stable, OMOP-named join target for care-site context at
whichever grain they need. Resolves
...
- **location** (`care_site_type = 'location'`) — a Tamanu `location` (room/bed). The care
...
**Why a wrapper.** Tamanu stores care-site structure as `departments` and `locations`,
each linked to a `facilities` row. `ref__care_site` gives downstream models a typed,
OMOP-named surface (`care_site_id`, `care_site_type`, `care_site_name`,
...
`clinical__visit_detail` key `care_site_id` on the **location** of the encounter's /
segment's location — Tamanu always records a location on an encounter, so `care_site_id`
is always populated. A location's grouping (`location_group`, the physical
"clinic"/"ward") is a separate, coarser concept: consumers that need it (e.g.
`ds__outpatient_visit`) join `location_groups` directly.

**Who reads it.** `clinical__visit_occurrence` and `clinical__visit_detail` (both
`care_site_id` FK → location-type rows); `metric__` / `dataset__` models that disaggregate
by facility (see the `hypertension_controlled` worked example in D5, which joins
...
and location ids occupy distinct UUID spaces. Soft-deleted rows are filtered by the base
...
- **BL-001:** One row per care site, sourced from `{{ ref('departments') }}`,
  `{{ ref('locations') }}`, and `{{ ref('facilities') }}` only (D10) — never
  `public.*`. Soft-delete filtering is inherited from the base models. The department and
  location id spaces are disjoint, so the union preserves a unique `care_site_id`;
...
- **BL-002:** OMOP column naming is applied — `id → care_site_id`, `name → care_site_name`,
  `code → care_site_source_value` — for both departments and locations. The parent
...
- **BL-005:** The model is the `union all` of two grains — departments
  ...
  **location-type** rows (the encounter's / segment's own location).
...
| AC-006 | A location denormalises into a `care_site_type='location'` row carrying its facility | BL-002, BL-005 | dbt unit test (`test_ref__care_site_location_denormalises_facility`) |
...
| `locations` | `bases/` | Location care sites (id, code, name) and parent facility link |
...
| Model | Use |
|---|---|
| `clinical__visit_occurrence` | `care_site_id` FK → location-type rows (AC-008 there) |
| `clinical__visit_detail` | `care_site_id` FK → location-type rows (the segment's own location) |
