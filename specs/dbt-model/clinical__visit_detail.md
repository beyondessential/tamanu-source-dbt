...
- **BL-002:** `encounter_history` is a single timeline where each row is a full snapshot
  ...
  defaulting to the segment's own start (AC-013). Ordering breaks ties on `visit_detail_id`
...
| AC-013 | The final segment of an encounter that has not closed (`end_datetime` is NULL) has a NULL `visit_detail_end_datetime` / `visit_detail_end_date`, not the segment's own start | BL-002 | dbt unit test (`test_clinical__visit_detail_open_encounter_stays_open`) |
