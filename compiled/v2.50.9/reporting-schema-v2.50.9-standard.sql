...
with price_pivot as (
    select
        invoice_product_id
        , max(case when invoice_price_list_id = 'Pricing-DisabledBehavioralHealth' then price end)
            as price_1
        , max(case when invoice_price_list_id = 'Pricing-ForeignWorker' then price end)
            as price_2
        , max(case when invoice_price_list_id = 'Pricing-Inmate' then price end)
            as price_3
        , max(case when invoice_price_list_id = 'Pricing-MHHSCareProvider' then price end)
            as price_4
        , max(case when invoice_price_list_id = 'Pricing-PalauanPensioner' then price end)
            as price_5
        , max(case when invoice_price_list_id = 'Pricing-PalauanRegular' then price end)
            as price_6
        , max(case when invoice_price_list_id = 'Pricing-Tourist' then price end)
            as price_7
        , max(case when invoice_price_list_id = 'Pricing-Volunteer' then price end)
            as price_8
        , max(case when invoice_price_list_id = 'Pricing-YapLocal' then price end)
            as price_9
...
insurance_pivot as (
    select
        invoice_product_id
        , max(case when invoice_insurance_plan_id = 'insurer-calvos' then coverage_value end)
            as cov_1
        , max(case when invoice_insurance_plan_id = 'insurer-MHHS' then coverage_value end)
            as cov_2
        , max(case when invoice_insurance_plan_id = 'insurer-MSA' then coverage_value end)
            as cov_3
        , max(case when invoice_insurance_plan_id = 'insurer-NHI' then coverage_value end)
            as cov_4
        , max(case when invoice_insurance_plan_id = 'insurer-netCare' then coverage_value end)
            as cov_5
        , max(case when invoice_insurance_plan_id = 'insurer-stayWell' then coverage_value end)
            as cov_6
        , max(case when invoice_insurance_plan_id = 'insurer-takeCare' then coverage_value end)
            as cov_7
...
select
    ...
    , pp.price_1 as "Price: Disabled or Behavioral Health"
    , pp.price_2 as "Price: Foreign Worker"
    , pp.price_3 as "Price: Inmate"
    , pp.price_4 as "Price: MHHS Care Provider"
    , pp.price_5 as "Price: Palauan Pensioner"
    , pp.price_6 as "Price: Palauan Regular"
    , pp.price_7 as "Price: Tourist"
    , pp.price_8 as "Price: Volunteer"
    , pp.price_9 as "Price: Yap Local"
    , case
...
create or replace view "reporting"."ds__sensitive_imaging_requests" as (
...
create or replace view "reporting"."ds__sensitive_encounters_emergency" as (
...
join "reporting"."patients" p on p.id = e.patient_id
left join "reporting"."locations" l on l.id = e.location_id
join "reporting"."facilities" f on f.id = l.facility_id and f.is_sensitive = True
left join "reporting"."reference_data" village on village.id = p.village_id
left join "reporting"."reference_data" arrival_mode on arrival_mode.id = t.arrival_mode_id
left join "reporting"."reference_data" chief_complaint on chief_complaint.id = t.chief_complaint_id
left join "reporting"."reference_data" secondary_complaint on secondary_complaint.id = t.secondary_complaint_id
...
create or replace view "reporting"."ds__sensitive_referrals" as (
...
create or replace view "reporting"."ds__sensitive_outpatient_appointments_audit" as (
...
create or replace view "reporting"."ds__sensitive_location_bookings" as (
...
create or replace view "reporting"."ds__sensitive_procedures" as (
...
create or replace view "reporting"."ds__sensitive_medication_dispenses" as (
...
create or replace view "reporting"."ds__sensitive_encounter_diets" as (
...
create or replace view "reporting"."ds__sensitive_user_audit" as (
...
create or replace view "reporting"."ds__sensitive_diagnoses" as (
...
create or replace view "reporting"."ds__sensitive_vaccinations" as (
