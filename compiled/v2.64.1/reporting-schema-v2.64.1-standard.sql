...
comment on schema reporting is '2.64.1';
...
select
    a.id,
    a.created_at at time zone 'Australia/Sydney' as created_datetime,
...
create or replace view "reporting"."outpatient_appointments_change_events" as (
...
create or replace view "reporting"."tasks" as (
select
    id,
    encounter_id,
    name,
...
create or replace view "reporting"."task_designations" as (
select
    id,
    task_id,
    designation_id
from "public"."task_designations"
where deleted_at is null
);
...
create or replace view "reporting"."map__lab_test_result_encoding" as (
...
with appointment_changes as (
    ...
    from "reporting"."outpatient_appointments_change_events" c
)
...
create or replace view "reporting"."clinical__procedure_occurrence" as (
...
with procedures as (
    select * from "reporting"."procedures"
),

imaging_requests as (
    select * from "reporting"."imaging_requests"
),
...
imaging_branch as (
    ...
    case
        ...
        when ir.imaging_type = 'orthopantomography' then 'Orthopantomography'
        else ir.imaging_type
    end
 as procedure_source_name,
...
patient_data as (
    ...
    left join "reporting"."locations" planned_location
...
from appointment_changes
    ...
    join "reporting"."outpatient_appointments" a on a.id = cl.appointment_id
    where
...
left join (
    ...
    from "reporting"."location_groups" lg2
    join "reporting"."facilities" f2 on f2.id = lg2.facility_id
...
create or replace view "reporting"."ds__task_followup" as (
...
task_designation_agg as (
    ...
    from "reporting"."task_designations" td
    left join "reporting"."reference_data" designation on designation.id = td.designation_id
    group by td.task_id
),
...
followup_appointments as (
    ...
    join "reporting"."outpatient_appointments" a
...
select
    ...
    case
        when exists (
                ...
                join "reporting"."user_designations" ud
...
join "reporting"."patients" p on p.id = ae.patient_id
left join "reporting"."reference_data" village on village.id = p.village_id
left join "reporting"."reference_data" not_completed_reason
    on not_completed_reason.id = t.not_completed_reason_id
left join "reporting"."users" requester on requester.id = t.requested_by_user_id
left join "reporting"."discharges" d on d.encounter_id = ae.encounter_id
left join "reporting"."reference_data" disposition on disposition.id = d.disposition_id
...
numbered as (
    select
        ...
        dl.start_datetime,
        dl.type,
        row_number() over (partition by dl.encounter_id order by dl.start_datetime, dl.id)
    - row_number() over (partition by dl.encounter_id, dl.department_id order by dl.start_datetime, dl.id)
...
numbered as (
    select
        ...
        ll.start_datetime,
        ll.type,
        row_number() over (partition by ll.encounter_id order by ll.start_datetime, ll.id)
    - row_number() over (partition by ll.encounter_id, ll.location_id order by ll.start_datetime, ll.id)
...
lab_result_encoding as (
    select * from "reporting"."map__lab_test_result_encoding"
),
...
create or replace view "reporting"."int__vaccine_administration_attributes" as (
...
create or replace view "reporting"."metric__birth" as (
...
create or replace view "reporting"."metric__ipd_procedure" as (
-- metric__ipd_procedure -- D5 metric view for the IPD-scoped procedure indicator registered in
-- documentations/metrics/*.yml: ipd_procedure.
--
-- Per-procedure (subject) grain: one row per recorded procedure performed during an inpatient
-- admission, value_numeric 1, so a consumer aggregates at whatever grain it needs. See
-- specs/dbt-model/metric__ipd_procedure.md BL-001 for why this is its own metric.
--
-- "Inpatient" is OMOP concept 9201, the same definition metric__inpatient_visit-style concepts
-- use -- it covers only the admission encounter type (models/maps/map__omop_visit_type.sql).
...
with procedure_occurrence as (
    ...
    select * from "reporting"."clinical__procedure_occurrence"
...
select
    'ipd_procedure'::text as metric_id,
    null::text as variant_id,
...
create or replace view "reporting"."metric__opd_imaging_request" as (
...
imaging_results as (
    select * from "reporting"."imaging_results"
),

imaging_request_areas as (
    select * from "reporting"."imaging_request_areas"
),
...
create or replace view "reporting"."metric__opd_procedure" as (
...
create or replace view "reporting"."metric__pharmacy_order" as (
...
create or replace view "reporting"."metric__procedure" as (
...
create or replace view "reporting"."metric__immunisation_dose" as (
...
with  __dbt__cte__int__vaccine_administration_attributes as (
-- int__vaccine_administration_attributes -- one row per bases/vaccine_administrations row,
-- any status, carrying every attribute both immunisation metrics disaggregate by: facility,
-- antigen, schedule dose position, sex, EPI age-cohort input, and patient village. Also
-- resolves the not-given reason, populated only for a NOT_GIVEN row.
...
join person pr
    ...
    select * from "reporting"."clinical__drug_exposure"
    where drug_exposure_type_source_value = 'vaccination'
),

vaccine_attributes as (
    select * from __dbt__cte__int__vaccine_administration_attributes
),
...
select
    ...
    case
        when age_months between 0 and 11    then '<1 year'
        when age_months between 12 and 23   then '12-23 months'
        when age_months between 24 and 59   then '24-59 months'
        when age_months between 60 and 1440 then '5+ years'
        else 'Unknown age'
    end
 as age_group__who_epi_schedule,
...
create or replace view "reporting"."metric__immunisation_refusal" as (
...
join person pr
    ...
    select * from "reporting"."clinical__observation"
...
create or replace view "reporting"."metric__ipd_diagnosis" as (
-- metric__ipd_diagnosis -- D5 metric view for the IPD-scoped diagnosis indicator registered in
-- documentations/metrics/*.yml: ipd_diagnosis.
--
-- BL-001: the registry carries the definition; this model is its implementation.
-- See specs/dbt-model/metric__ipd_diagnosis.md for BL-001..BL-007.
...
diagnosis_ipd_segment as (
    ...
    join visit_detail vd
        ...
        and vd.visit_detail_concept_id = 9201 -- OMOP 'Inpatient Visit'
...
case
 ...
 as age_years
    from condition_occurrence cco
    join diagnosis_ipd_segment dis
        on dis.condition_occurrence_id = cco.condition_occurrence_id
    join person pr
        on pr.person_id = cco.person_id
    join locations loc
        on loc.id = dis.care_site_id
)

select
    'ipd_diagnosis'::text as metric_id,
    null::text as variant_id,
    condition_occurrence_id::varchar as subject_id,
...
create or replace view "reporting"."metric__opd_diagnosis" as (
