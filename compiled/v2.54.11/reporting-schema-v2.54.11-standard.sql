...
with price_pivot as (
    select
        invoice_product_id
        , max(case when invoice_price_list_id = 'invoicePriceList-ChuukNonResPricing' then price end)
            as price_1
        , max(case when invoice_price_list_id = 'invoicePriceList-ChuukResPricing' then price end)
            as price_2
        , max(case when invoice_price_list_id = 'invoicePriceList-KosraeNonResPricing' then price end)
            as price_3
        , max(case when invoice_price_list_id = 'invoicePriceList-KosraeResPricing' then price end)
            as price_4
        , max(case when invoice_price_list_id = 'invoicePriceList-PohnpeiPricing' then price end)
            as price_5
        , max(case when invoice_price_list_id = 'invoicePriceList-PohnpeiPricing65' then price end)
            as price_6
        , max(case when invoice_price_list_id = 'invoicePriceList-YapResPricingChild' then price end)
            as price_7
        , max(case when invoice_price_list_id = 'invoicePriceList-YapNonResPricing' then price end)
            as price_8
        , max(case when invoice_price_list_id = 'invoicePriceList-YapResPricing' then price end)
            as price_9
        , max(case when invoice_price_list_id = 'invoicePriceList-YapResPricingSen' then price end)
            as price_10
...
insurance_pivot as (
    select
        invoice_product_id
        , max(case when invoice_insurance_plan_id = 'insuranceProduct-MicareBasic' then coverage_value end)
            as cov_1
...
select
    ...
    , pp.price_1 as "Price: Chuuk Non-Resident"
    , pp.price_2 as "Price: Chuuk Resident"
    , pp.price_3 as "Price: Kosrae Non-Resident"
    , pp.price_4 as "Price: Kosrae Resident"
    , pp.price_5 as "Price: Pohnpei"
    , pp.price_6 as "Price: Pohnpei Senior"
    , pp.price_7 as "Price: Yap 0-12"
    , pp.price_8 as "Price: Yap Non-Resident"
    , pp.price_9 as "Price: Yap Resident"
    , pp.price_10 as "Price: Yap Senior"
    , case
