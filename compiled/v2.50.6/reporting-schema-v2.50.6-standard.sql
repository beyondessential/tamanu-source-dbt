...
with price_pivot as (
    select
        invoice_product_id
        , max(case when invoice_price_list_id = 'invoicePriceList-LocalPricing' then price end)
            as price_1
        , max(case when invoice_price_list_id = 'invoicePriceList-PrivatePricing' then price end)
            as price_2
        , max(case when invoice_price_list_id = 'invoicePriceList-VisitorPricing' then price end)
            as price_3
...
insurance_pivot as (
    select
        invoice_product_id
        , max(case when invoice_insurance_plan_id = 'insuranceProduct-InsurancePlus' then coverage_value end)
            as cov_1
        , max(case when invoice_insurance_plan_id = 'insuranceProduct-Medicare' then coverage_value end)
            as cov_2
...
select
    ...
    , pp.price_1 as "Price: Local"
    , pp.price_2 as "Price: Private"
    , pp.price_3 as "Price: Visitor"
    , case
    ...
    end as "Insurance: Insurance Plus"
    , case
        when ip.insurable = false then 'n/a'
        else cast(
            coalesce(
