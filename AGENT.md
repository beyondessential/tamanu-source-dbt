...
- A report model's column names are its translated labels, so a unit test fixture
  asserting report output names the standard label literally. Fixtures render without the
  macro namespace, so `translate_label()` cannot be called in a `given` or `expect` block.
  A deployment that localises one of those columns excludes this package's unit tests from
  its own runs; the fixture is not parameterised to follow it
