...
- Source: `report_translations_standard.csv` → generates `macros/translation.sql`
...
Run all script tests from the project root (pytest config in `pyproject.toml` sets `pythonpath = ["scripts"]`):
...
Test files:
- `scripts/tests/test_propagate_patch.py` — covers `propagate_patch.py`
- `scripts/tests/test_translation_utils.py` — covers `scripts/utils/translation_utils.py`
...
| Translation macro | `macros/translations.sql` (hand-authored); `macros/translation.sql` is generated from the CSV — do not edit directly |
