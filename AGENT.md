...
- `macros/intermediate/` — utilities used by models under `models/intermediate/`
- `macros/reports/` — `<report>_report(...)` macros called by SQL files under `models/reports/sql/{standard,sensitive}/`

Cross-cutting utilities (`datetime.sql`, `translations.sql`, `parameter.sql`, etc.) stay at the top of `macros/`. A shared SQL fragment used across more than one layer belongs there too, not in whichever layer folder happened to need it first — `invoice_item_amounts.sql` is the precedent. A macro owned by a single layer sits beside its callers in that layer's folder.

Naming: `<entity>_dataset` in `macros/datasets/`, `<report>_report` in `macros/reports/`, and `<entity>_core` for a body macro shared by a dataset and a report. For *when* to reach for each of these — and when a plain `ref()` is the right answer instead — see "Reuse: picking a sharing mechanism" in [`dbt-conventions.md`](.maui/knowledge/standards/dbt-conventions.md). Word order between a report macro and its dataset is not always consistent — `encounter_invoice_audit_report` reads `ds__encounter_invoices`, and `audit_outpatient_appointments_report` pairs with `outpatient_appointments_audit_dataset`. Match the existing name of whatever you are extending rather than inventing a regular form.
...
When the standard and sensitive variants share non-trivial SELECT/WHERE logic, extract it into a macro at `macros/reports/<name>.sql` parameterised on `is_sensitive`, and reduce each SQL file to a single `{{ <name>_report(is_sensitive=...) }}` call. The macro picks `ds__<name>` vs `ds__sensitive_<name>` based on the flag. See `admissions_line_list_report` for the pattern.

Most reports need nothing beyond a plain `ref()` — see "Reuse: picking a sharing mechanism" in [`dbt-conventions.md`](.maui/knowledge/standards/dbt-conventions.md) before reaching for a macro. In particular, `encounter_summary_report` is **not** an example of the pattern above: it has no dataset and builds directly off base models.

Never edit `list_tamanu_reports.md` — it is auto-generated by `scripts/list_tamanu_reports.py`.

## Translation system

- Source: `csv/report_translations_standard.csv` → generates `macros/default_translations.sql`
...
- Unit test fixtures render without the macro namespace, so `translate_label()` cannot be
  called in a `given` or `expect` block. Where a fixture asserts a translated column alias,
  parameterise it with a var defaulting to the standard label — the patient identifier uses
  `{{ var('patient_display_id_label', 'Patient ID') }}`. A deployment that overrides
  `report.reporting.patientDisplayId` (Samoa `NHN`, Nauru `MRID`) sets the matching var in
  its own `dbt_project.yml`, or every unit test asserting that column fails with
  `column "<label>" does not exist`

## Metric definitions registry

The canonical metric/derived-element registry follows the same generation
pattern as translations so it ships as a **view** in the compiled production
bundle rather than a seed table:

- Source: `csv/metric_definitions.csv` → generates `macros/metric_definitions.sql`
  (the `get_metric_definitions()` macro emitting an inline `values` relation)
- Model: `models/metric_definitions.sql` is a thin view calling the macro;
  consume it everywhere via `{{ ref('metric_definitions') }}`
- Run `python scripts/generate_metric_definitions_macro.py` if the CSV changes
- `macros/metric_definitions.sql` is auto-generated — never edit it directly
- Deployment-specific variants live in
...
| Translation source | `csv/report_translations_standard.csv` |
